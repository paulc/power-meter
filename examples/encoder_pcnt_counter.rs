...
use power_monitor::encoder_pcnt::encoder_module_init;
...
async fn main(spawner: Spawner) -> ! {
    ...
    // Rotary encoder
    let (encoder0, _encoder1) = encoder_module_init(peripherals.PCNT);
    ...
    let counter = encoder0
        .init_counter(spawner, clk, dt, Duration::from_millis(100))
        .await;
    defmt::info!(">> ENCODER0 INIT");

    loop {
        let n = counter.wait().await;
        defmt::info!(">> Counter Value: {}", n);
    }
}
